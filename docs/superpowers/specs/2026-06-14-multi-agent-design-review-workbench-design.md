...
第一阶段不依赖 LLM 或 runner 自动生成澄清需求。系统只提供模板和上下文展示，Human 根据原始需求、Agent 问题和 Human 回答编辑并确认 `input/clarified_requirement.md`。

`input/clarified_requirement.md` 是进入 Draft Design 阶段的权威需求输入。

### 5.4 Draft Design

Architect 和 Engineer 在隔离上下文中各自产出设计初稿。
...
系统开放所有初稿。Architect 和 Engineer 互评对方初稿，Reviewer 对所有初稿提供专职评审。
...
Architect 和 Engineer 读取自己收到的评审意见、Human 评论和 Human 决策，提交修订版。
...
第一阶段不依赖 LLM 或 runner 自动合成最终文档。Synthesizer 与其他 Agent 一样走 prompt 到 response 的提交流程。

系统生成 `agents/synthesizer/synthesis_prompt.md`，Human 将该 prompt 交给外部 Agent 或手动编写结果，再通过 Web UI 粘贴或文件投递提交到：
...
- 能生成并展示 Architect 和 Engineer 的 draft prompt。
